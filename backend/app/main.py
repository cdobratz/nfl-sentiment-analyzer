...
async def get_sentiment_analyzer():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to initialize sentiment analyzer")
...
async def get_twitter_service():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to initialize Twitter service")
...
async def get_weekly_sentiment(
    ...
    try:
        ...
        confidence = sentiment_df['confidence'].mean() if 'confidence' in sentiment_df else None
...
async def get_sentiment_trends(
    ...
    try:
        ...
        tweets = await twitter_service.get_historical_tweets(game_id, start_date, end_date)
...
async def get_game_sentiment_details(
    ...
    try:
        ...
        home_team_tweets = sentiment_df[sentiment_df['text'].str.contains(tweets[0]['home_team'], case=False)]
        away_team_tweets = sentiment_df[sentiment_df['text'].str.contains(tweets[0]['away_team'], case=False)]
        ...
        return GameSentimentDetails(
            ...
            top_tweets=sentiment_df.nlargest(5, 'score')[['text', 'score', 'author']].to_dict('records'),
            updated_at=datetime.now().isoformat()
        )
