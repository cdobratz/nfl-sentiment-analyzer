...
class DataPreprocessor:
    ...
    def calculate_team_stats(self, historical_games: List[Dict[str, Any]], team_id: str, 
                           last_n_games: int = 5) -> Dict[str, float]:
        ...
        try:
            ...
            stats = {
                ...
                'points_scored_avg': np.mean([g['points_scored'] for g in recent_games]),
                'points_allowed_avg': np.mean([g['points_allowed'] for g in recent_games]),
    ...
    def process_sentiment_data(self, sentiment_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            ...
            home_sentiment = np.mean([t.get('sentiment_score', 0) for t in home_tweets]) if home_tweets else 0
            away_sentiment = np.mean([t.get('sentiment_score', 0) for t in away_tweets]) if away_tweets else 0
    ...
    def prepare_game_features(self, game_data: Dict[str, Any], 
                            historical_games: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Prepare features for a single game prediction."""
        try:
            # Get team stats
            home_stats = self.calculate_team_stats(historical_games, game_data['homeTeam']['id'])
            away_stats = self.calculate_team_stats(historical_games, game_data['awayTeam']['id'])

            # Process sentiment data
            sentiment_features = self.process_sentiment_data(game_data.get('sentiment_data', {}))
            ...
            features = {
                ...
                'home_yards_per_play': home_stats['yards_per_game'] / 60,  # Approximate plays per game
                ...
                'analyst_confidence_home': sentiment_features['analyst_confidence_home'],
                'analyst_confidence_away': sentiment_features['analyst_confidence_away'],
